...
const updateBlacklist = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ msg: "Blacklist updated", blacklisted_ingredients });
  } catch (error) {
  ...
  try {
    ...
    if (newPreferences && userId) {
      const { preferences } = await db.one(
        `UPDATE preferences SET preferences=$2 WHERE user_id=$1 RETURNING preferences`,
        [userId, newPreferences],
      );
      return res
        .status(201)
        .json({ msg: "Preferences updated successfully", preferences });
    } else {
...
const updateFavoriteRecipes = async (req, res) => {
  try {
    ...
    const { favorited_recipes } = (await db.oneOrNone(
      `SELECT favorited_recipes FROM preferences WHERE user_id=$1`,
      [userId],
    )) || {
      favorited_recipes: [],
    };
    ...
    if (recipe.isFavorited) {
      ...
      const alreadyFavorited = favorited_recipes.find(
        (rec) => String(rec.id) + rec.title === String(recipe.id) + rec.title,
      );
      ...
      newFavorited = favorited_recipes.filter(
        (rec) => String(rec.id) + rec.title !== String(recipe.id) + rec.title,
      );
    }
    ...
    await db.none(
      `UPDATE preferences SET favorited_recipes = $2::jsonb WHERE user_id=$1`,
      [userId, jsonNewFavorited],
    );
...
export { updateFoodPref, updateBlacklist, updateFavoriteRecipes, getBlacklist };
